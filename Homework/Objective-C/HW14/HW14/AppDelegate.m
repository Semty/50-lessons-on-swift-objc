...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    NSString *firstHalfOfTheText = [text substringToIndex:[text length] / 2];
    
    NSString *secondHalfOfTheText = [text substringFromIndex:[text length] / 2];
    ...
    for (NSString *sentence in sentences) {
        ...
        NSRange ourString = [sentence rangeOfString:@"NSString"];
        
        if (ourString.location != NSNotFound) {
    ...
    for (NSString *sentence in result) {
        
        NSArray *tempString = [sentence componentsSeparatedByString:@" "];
        NSMutableArray *tempReversedSentence = [[NSMutableArray alloc] init];
        
        for (NSString *reversedWord in [tempString reverseObjectEnumerator]) {
            
            [tempReversedSentence addObject:reversedWord];
        }
        NSString *tempResultSentence = [tempReversedSentence componentsJoinedByString:@" "];
        
        [reverseResult addObject:tempResultSentence];
    }
    ...
    NSArray *arrayOfTheText = [text componentsSeparatedByString:@" "];
    NSMutableArray *resultWords = [[NSMutableArray alloc] init];
    
    for (NSString *word in arrayOfTheText) {
        ...
        for (NSUInteger i = 0; i < [word length]; i++) {
            ...
            [tempWord addObject:letter];
        }
        
        tempWord[0] = [tempWord[0] lowercaseString];
        tempWord[[tempWord count] - 1] = [tempWord[[tempWord count] - 1] uppercaseString];
        
        NSString *resultWord = [tempWord componentsJoinedByString:@""];
        
        [resultWords addObject:resultWord];
        
    }
    
    NSString *resultText = [resultWords componentsJoinedByString:@" "];
